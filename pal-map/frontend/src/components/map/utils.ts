...
// Most of the transformation constants and functions are adapted from the Palworld save editor
// Credits from https://github.com/oMaN-Rod/palworld-save-pal
...
// Convert Leaflet coordinates to map coordinates
export function leafletToMap(latlng: L.LatLng): { x: number; y: number } {
    // Convert from Leaflet to game coordinates
    const mapX = (latlng.lng - TRANSFORM_B) / TRANSFORM_A;
    const mapY = (latlng.lat - TRANSFORM_D) / TRANSFORM_C - 1;
    return { x: mapX, y: mapY };
}
